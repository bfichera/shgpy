...
class TestfFormGen(unittest.TestCase):
    ...
    def test_generate_uncontracted_fourier_transforms(self):
        shgpy.fformgen.generate_uncontracted_fourier_transforms(0.1745, 'tests/fform/uft10deg')
        shgpy.fformgen.generate_uncontracted_fourier_transforms_symb('tests/fform/uft_theta')
    ...
    def test_generate_contracted_fourier_transforms_complex(self):
        ...
        shgpy.fformgen.generate_contracted_fourier_transforms(save_filename, 'tests/fform/uft10deg', t_dip, t_quad, ndigits=4)
    ...
    def test_generate_contracted_fourier_transforms(self):
        ...
        t_dip = shgpy.make_tensor_real(shgpy.transform(shgpy.particularize(td.dipole['T_d']), shgpy.rotation_matrix_from_two_vectors(np.array([1, 1, 0]), np.array([0, 0, 1]))))
        t_quad = shgpy.make_tensor_real(np.zeros(shape=(3,3,3,3), dtype=object))
